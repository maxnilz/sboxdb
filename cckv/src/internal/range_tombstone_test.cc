...
struct Range {
  Range(std::string&& a, std::string&& b, Version v) {
    user_start_key_ = std::move(a);
    start_ = InternalKey(user_start_key_, v, kTypeRangeDeletion);

    std::string user_end_key = std::move(b);
    InternalKey end(user_end_key, v, kTypeRangeDeletion);
...
TEST(RangeTombstone, Fragment) {
  ...
  //    |---1----|  |---2-----|
  //  --a--------c--e---------g--
  cases.emplace_back("case 1", range_list{{"a", "c", 1}, {"e", "g", 2}}, "[a, c)@1, [e, g)@2");
  // Case 2
  //         |-----2-----|
  //   |----1---|
  // --a-----c--e--------g-------
  cases.emplace_back("case 2", range_list{{"a", "e", 1}, {"c", "g", 2}},
...
TEST(RangeTombstone, MaxCoveringSeq) {
  ...
  std::vector<std::tuple<std::string, Version, Version>> cases{
      {"a", 0, 0},
      // b
      {"b", 0, 0},
      {"b", 1, 1},
...
TEST(RangeTombstone, Iterator) {
  ...
  std::vector<std::pair<Version, std::vector<std::string>>> cases{
      {0, {}},
      {1, {"[b, d)@1", "[d, f)@1"}},
      {2, {"[b, d)@1", "[d, f)@1"}},
      {3, {"[b, d)@1", "[d, f)@3", "[f, h)@3", "[h, j)@3"}},
      {4, {"[b, d)@1", "[d, f)@4", "[f, h)@4", "[h, j)@4"}},
      {5, {"[b, d)@1", "[d, f)@4", "[f, h)@4", "[h, j)@5", "[j, l)@5"}},
      {6, {"[b, d)@1", "[d, f)@4", "[f, h)@4", "[h, j)@5", "[j, l)@5"}},
  };
...
TEST(RangeTombstone, PlayMaxCoveringBoundary) {
  ...
  for (auto& [version, expect] : cases) {
    auto pos = std::ranges::lower_bound(seqs.begin(), seqs.end(), version, std::greater<>{});
