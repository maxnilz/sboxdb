...
use syn::{
    Attribute, Data, DeriveInput, Fields, GenericParam, Generics, Ident, Index, LitStr, Meta,
    Token, Type, TypePath,
...
fn derive_visit(input: proc_macro::TokenStream, visit_type: &VisitType) -> proc_macro::TokenStream {
    ...
    let expanded = quote! {
        ...
        impl #impl_generics crate::sql::parser::#visit_trait for #name #ty_generics #where_clause {
             #[cfg_attr(feature = "recursive-protection", recursive::recursive)]
            fn visit<V: crate::sql::parser::#visitor_trait>(
                &#modifier self,
                visitor: &mut V
...
fn add_trait_bounds(mut generics: Generics, VisitType { visit_trait, .. }: &VisitType) -> Generics {
    for param in &mut generics.params {
        if let GenericParam::Type(ref mut type_param) = *param {
            type_param.bounds.push(parse_quote!(crate::sql::parser::#visit_trait));
        }
    }
    generics
}
        ...
        Data::Struct(data) => match &data.fields {
                    ...
                    if is_option && attributes.with.is_some() {
                        ...
                        quote_spanned!(f.span() =>
                            if let Some(value) = &#modifier self.#name {
                                #pre_visit crate::sql::parser::#visit_trait::visit(value, visitor)?; #post_visit
                            }
                        )
                    } else {
                        let (pre_visit, post_visit) = attributes.visit(quote!(&#modifier self.#name));
                        quote_spanned!(f.span() =>
                            #pre_visit crate::sql::parser::#visit_trait::visit(&#modifier self.#name, visitor)?; #post_visit
                        )
                    }
                });
                quote! {
            ...
            Fields::Unnamed(fields) => {
                let recurse = fields.unnamed.iter().enumerate().map(|(i, f)| {
                    ...
                    quote_spanned!(f.span() => #pre_visit crate::sql::parser::#visit_trait::visit(&#modifier self.#index, visitor)?; #post_visit)
                });
                quote! {
                    #(#recurse)*
                }
                ...
                match &v.fields {
                    Fields::Named(fields) => {
                        ...
                        let visit = fields.named.iter().map(|f| {
                            ...
                            quote_spanned!(f.span() => #pre_visit crate::sql::parser::#visit_trait::visit(#name, visitor)?; #post_visit)
                        });
